...
class _SampleWorldAabbs:
    ...
    def remember_cached(self, world, geometry):
...
class PlaceSceneChecker:
    ...
    def sample(self, q, aperture, loaded):
        from erc_phase1_solution.geometry_array_checks import any_boolean
        node = self.node
        ...
        if key in self.cache:
            self.cache_hits += 1
            return True
        world_cache = self._world_geometry_cache
        ...
        for mesh in node.carried_collision_meshes:
            ...
            cached = (world_cache.capture(local_mesh, surface, transform)
            ...
            if type(self.tool) is ShelfCradleGeometry:
                ...
                if local_mesh is not None:
                    ...
                    if type(local_mesh) is ModelLocalMesh and local_mesh.matches(surface):
                        ...
                        if type(self.table) is TableSceneObstacle:
                            table_surface = local_mesh
            if self.obstacle.intersects(bin_surface, hand, self.tool.watertight[link]):
                return self._reject('tool_bin', link=link)
            if self.table is not None and self.table.intersects(table_surface, hand, self.tool.watertight[link]):
                return self._reject('tool_table', link=link, solid=self.table.last_intersection)
            cached = (world_cache.capture(local_mesh, surface, hand)
                      if world_cache is not None else None)
            if cached is None:
        ...
        if self.screen is not None:
            ...
            for link, surface in {**robot, **tool}.items():
                if link in ('head_1_link', 'head_2_link', 'head_front_camera_link'):
                    continue
                if (any_boolean(world_aabbs.reduce(surface, 'max') < low)
                        or any_boolean(high < world_aabbs.reduce(surface, 'min'))):
                    continue
        ...
        if (type(self.obstacle) is NominalBinObstacle
            ...
            snapshot_options = ({'_world_geometry': world_aabbs.snapshot_geometry(robot)}
        ...
        for link, surface in tool.items():
            ...
            for other, other_surface in robot.items():
                ...
                if any_boolean(high < other_low) or any_boolean(other_high < low):
                    continue
...
def plan_scene_checked_place(node, positions, rotation, carried_start, torso_ready,
                             ...
                             geometry_backend=None, release_only_request=None):
    ...
    if tool is None:
        tool = ShelfCradleGeometry(urdf, resolve_package, immutable_local=True)
