...
class ManipulationNode(Node):
    ...
    def _move_torso(self, height: float, duration: float = 2.0) -> bool:
        return self._follow(
            self.torso_client,
            ['torso_lift_joint'],
            [height],
    ...
    def _recover_closed_place(
        ...
        *,
        ...
        if recovery_ok and ensure_place_torso:
            recovery_ok = self._best_effort(
                lambda: self._move_torso(self.place_torso_height, 2.2)
            )
    ...
    def _place(self, payload=None) -> bool:
        ...
        torso_ready[0] = self.place_torso_height
        place_staging = self._carried_staging_solution.copy()
        place_staging[0] = self.place_torso_height
        gravity_supported = bool(
            getattr(self, '_gravity_supported_payload', False)
        )
