...
class EmptyPickupCollision:
    ...
    def capture(cls, node):
        ...
        if geometry is None:
            geometry = ShelfCradleGeometry(urdf, get_package_share_directory)
    ...
    def _sample_uncached(self, q, aperture):
        # Preserve the existing body/static verdict caches and their policy.
        ...
        bounds = {link: _bounds(surface) for link, surface in {**robot, **tool}.items()}
        prepared = {}
        for link in robot:
