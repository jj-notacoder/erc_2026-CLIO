...
class ManipulationNode(Node):
    ...
    def __init__(self) -> None:
        ...
        if (not np.isfinite(self.lift_first_extraction_lift_m)
                or not 0.0 < self.lift_first_extraction_lift_m <= .005):
            raise ValueError('lift_first_extraction_lift_m must be positive and at most 5 mm')
        if not (
        ...
        self.create_subscription(Odometry, '/odom', self._on_staging_odom, SENSOR_QOS)
        ...
        self.create_subscription(
            ...
            self._on_target_tracking,
            RELIABLE_QOS,
        )
        self.create_subscription(
            String,
            '/erc/perception/target_book_tracking_status',
            self._on_target_tracking_status,
            RELIABLE_QOS,
        )
        self.create_subscription(
            PointStamped,
        ...
        self.create_subscription(
            String, '/erc/perception/status', self._on_bin_status,
            TRANSIENT_RELIABLE_QOS,
        )
        self.create_subscription(
            String,
        ...
        self.create_timer(1.0, self._announce_ready)
        self.create_timer(0.05, self._monitor_held_payload)
    ...
    def _head_hold_snapshot_locked(self, target, completed):
        ...
        if any(stamps[n] <= completed[1] for n in HEAD_JOINTS):
            return None
        if any(abs(joints[n] - float(v)) > 1e-6 for n, v in zip(HEAD_JOINTS, target)):
            return None
    ...
    def _try_completed_head_hold(self, target, completed):
        """None means normal motion; False means an attempted hold was unsafe."""
        ...
        with self._adaptive_command_guard():
            # Capture an epoch before helpers that independently lock sensors.
            # A delivered contact/feedback update may require the original
            # motion path; it can never approve a mixed-snapshot head hold.
            checked = self._head_hold_snapshot(target, completed)
            if checked is None or not self._head_hold_admission_clear():
                return False
            with self._lock:
                latest = self._head_hold_snapshot_locked(target, completed)
                if latest is None:
                    return False
                if (latest['contact_epoch'] != checked['contact_epoch']
                        or latest['contact_epoch'] != snapshot['contact_epoch']):
                    return False
                if (self._cancel.is_set() or getattr(self, '_goal_handles', ())
                        or getattr(self, '_pending_retained_acceptances', ())
                        or getattr(self, '_payload_hazard_latched', None) is not None
                        or getattr(self, '_held_grip_sensor_fault', None) is not None
                        or getattr(self, '_target_robot_contact_latched', False)):
                    return False
                if any(abs(latest['joints'][n] - value) > 1e-6
                       for n, value in snapshot['joints'].items()):
                    return False
                attached, current = snapshot['attached'], latest['attached']
                if ((attached is None) != (current is None)
                        or (attached is not None and not np.array_equal(attached, current))):
                    return False
                if (latest['contact_generation'] != checked['contact_generation']
                        or latest['feedback_identity'] is not checked['feedback_identity']):
                    # Fresh sensor delivery invalidates only this no-command
                    # shortcut. Never accept mixed epochs: the caller must run
                    # its original complete head sweep and controller action.
                    if latest['feedback_identity'] is not checked['feedback_identity']:
                        ...
                        if (feedback is None or not all(math.isfinite(getattr(feedback, name, math.nan))
                            ...
                            return False
                    return None
                # Preserve the original controller-success epoch. No sensor
                # helper or publisher runs while this plain Lock is held.
    ...
    def _move_arm_solution(self, solution: Sequence[float], duration: float) -> bool:
        q = np.asarray(solution, dtype=float)
        return self._follow(self.arm_client, ARM_JOINTS, q[1:], duration)
    ...
    def _pick(self, payload: Optional[Dict] = None) -> bool:
        ...
        if lift_enabled:
            ...
            if getattr(self, '_shelf_cradle_geometry', None) is None:
                urdf = Path(get_package_share_directory('erc_description')) / 'urdf' / 'tiago_pro.urdf'
                self._shelf_cradle_geometry = ShelfCradleGeometry(urdf, get_package_share_directory)
        ...
        if empty_setup_guard is not None:
            empty_setup_guard.require_fresh(empty_setup_guard.start, empty_setup_guard.initial_aperture)
        ...
        if correlation is not None:
            ...
            def verify_open():
                return observe_measured_open_pose(self, correlation, solutions[-1],
                    'placement_open_measured')['verified']
